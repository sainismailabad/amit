...
window.Foodz = {
    ...
    saveCartToStorage: function() {
        try { localStorage.setItem('foodz-cart', JSON.stringify(this.cart)); } catch (e) {}
    },

    loadCartFromStorage: function() {
        try {
            const saved = localStorage.getItem('foodz-cart');
            if (saved) {
    ...
    init: async function() {
        ...
        // Auto-redirect based on role - if already logged in, go to respective dashboard
        if (this.currentUser && authState.role) {
            ...
            // For 'customer' or unknown roles, stay on index.html
        }
        ...
        db.ref('notifications').orderByChild('userId').equalTo(this.currentUser.uid).on('child_added', (snapshot) => {
            if (!initialLoad) {
                ...
                if (!notif.read) {
                    ...
                    // Check for rating notification - redirect to orders page
                    if (notif.type === 'rating' && notif.message && notif.message.includes('show_rating_popup_')) {
                        const orderId = notif.message.replace('show_rating_popup_', '');
                        if (orderId) {
                            auth.markNotificationRead(snapshot.key);
                            const path = window.location.pathname;
                            if (!path.includes('customer-orders.html')) {
                                window.location.href = 'customer-orders.html?showRating=' + orderId;
                            } else {
                                // Already on orders page - rating modal will pick it up
                                if (window.RatingModal && typeof RatingModal.show === 'function') {
                                    setTimeout(() => RatingModal.show(orderId), 2000);
                                }
                            }
                            return;
                        }
    ...
    initLocation: async function() {
        ...
        try {
            const saved = localStorage.getItem('foodz-last-location');
            if (saved) {
    ...
    getCurrentLocation: function(showMessage = true) {
        ...
        navigator.geolocation.getCurrentPosition(
            async (pos) => {
                ...
                localStorage.setItem('foodz-last-location', JSON.stringify({ lat, lng, address: '' }));
                const address = await this.resolveAddressFromCoords(lat, lng);
                document.getElementById('checkout-address').value = address || '';
                localStorage.setItem('foodz-last-location', JSON.stringify({ lat, lng, address: address || '' }));
                if (status) {
    ...
    showAutocomplete: function(query) {
        ...
        const products = this._productSearchCache || [];
        const restaurants = this._restaurantsCache || {};
        ...
        const matches = products.filter(p => {
            ...
            return name.includes(q) || desc.includes(q) || ingredients.includes(q) || category.includes(q);
        });
        ...
        if (items.length === 0) {
            dropdown.innerHTML = `
                <div style="padding:16px;text-align:center;color:#888;font-size:0.9rem;">
                    <i class="fas fa-search me-1"></i> No matching items found
                </div>
            `;
        ...
        items.forEach(item => {
            ...
            const rest = restaurants[item.sellerId] || {};
            const restName = rest.name || 'Restaurant';
            const price = Number(item.price || 0).toFixed(2);
            
            // Highlight matching text
            const nameHtml = this._highlightMatch(item.name || 'Item', q);
            
            html += `
                <div onclick="Foodz.goToProduct('${item.id}')" style="display:flex;align-items:center;gap:12px;padding:10px 16px;cursor:pointer;border-bottom:1px solid #f5f5f5;transition:background 0.15s;" onmouseover="this.style.background='#f8f9fa'" onmouseout="this.style.background=''">
                    ${img ? `<img src="${img}" style="width:48px;height:48px;border-radius:8px;object-fit:cover;flex-shrink:0;" onerror="this.style.display='none'">` : `<div style="width:48px;height:48px;border-radius:8px;background:#f0f0f0;display:flex;align-items:center;justify-content:center;flex-shrink:0;"><i class="fas fa-utensils" style="color:#bbb;"></i></div>`}
                    <div style="flex:1;min-width:0;">
                        <div style="font-weight:600;font-size:0.9rem;color:#222;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${nameHtml}</div>
                        <div style="font-size:0.75rem;color:#888;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                            <i class="fas fa-store me-1" style="font-size:0.65rem;"></i>${restName}
                            <span style="margin:0 6px;color:#ddd;">|</span>
                            <span style="color:var(--primary);font-weight:600;">₹${price}</span>
                        </div>
                    </div>
        ...
        if (matches.length > maxResults) {
            html += `
                <div style="padding:10px 16px;text-align:center;color:var(--primary);font-size:0.8rem;font-weight:600;cursor:pointer;border-bottom:1px solid #f5f5f5;" onclick="document.getElementById('search-input').focus(); document.getElementById('autocomplete-dropdown').style.display='none'">
                    <i class="fas fa-search me-1"></i> View all ${matches.length} results
                </div>
            `;
        }
    ...
    _highlightMatch: function(text, query) {
        ...
        return text.substring(0, idx) + '<span style="background:#fff3cd;font-weight:700;">' + text.substring(idx, idx + query.length) + '</span>' + text.substring(idx + query.length);
    },
    ...
    loadCategories: function() {
        ...
        container.innerHTML = this.categories.map((cat, i) => `
            <div class="category-pill ${i===0?'active':''}" onclick="Foodz.filterByCategory('${cat}', this)">
                                            ...
                                            <div class="mt-2">
                                                ${qty > 0 ? `
                                                    <div class="d-flex align-items-center gap-2">
                                                         <button class="btn btn-sm btn-outline-primary rounded-circle" onclick="event.stopPropagation(); Foodz.updateCart('${p.id}', -1); Foodz.loadProductsNearbyIndex()">-</button>
                                                         <span class="fw-bold">${qty}</span>
                                                         <button class="btn btn-sm btn-outline-primary rounded-circle" onclick="event.stopPropagation(); Foodz.updateCart('${p.id}', 1); Foodz.loadProductsNearbyIndex()" ${!isOnline ? 'disabled' : ''}>+</button>
                                                    </div>
                                                ` : `
                                                    <button class="add-btn" onclick="event.stopPropagation(); Foodz.updateCart('${p.id}', 1); Foodz.loadProductsNearbyIndex()" ${stock <= 0 || !isOnline ? 'disabled' : ''}>${!isOnline ? 'Offline' : (stock <= 0 ? 'Sold Out' : '+ Add')}</button>
                                                `}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-8 col-sm-9">
                                        <div class="p-3">
                                            ...
                                            <div class="mt-3">
                                                ${qty > 0 ? `
                                                    <div class="d-flex align-items-center gap-2">
                                                         <button class="btn btn-sm btn-outline-primary rounded-circle" onclick="Foodz.updateCart('${p.id}', -1)">-</button>
                                                         <span class="fw-bold">${qty}</span>
                                                         <button class="btn btn-sm btn-outline-primary rounded-circle" onclick="Foodz.updateCart('${p.id}', 1)" ${!isOnline ? 'disabled' : ''}>+</button>
                                                    </div>
                                                ` : `
                                                    <button class="add-btn" onclick="Foodz.updateCart('${p.id}', 1)" ${stock <= 0 || !isOnline ? 'disabled' : ''}>${!isOnline ? 'Offline' : (stock <= 0 ? 'Sold Out' : '+ Add')}</button>
                                                `}
                                            </div>
                                        </div>
                                    </div>
                ...
                return `
                    <div class="col-lg-3 col-md-4 col-sm-6 mb-4">
                        <div class="restaurant-card" data-id="${r.id}" style="cursor:${isOnline ? 'pointer' : 'not-allowed'}; position: relative; ${offlineStyle}">
                            ...
                            <div class="card-body" ${isOnline ? `onclick="Foodz.openRestaurantDetailModal('${r.id}')"` : ''}>
                                <h5>${safeName}</h5>
    ...
    shareRestaurant: async function(restId) {
        ...
        const text = `Check out ${r.name || 'this restaurant'} on Ibad Foodz!`;
        if (navigator.share) {
            navigator.share({ title: r.name || 'Restaurant', text, url });
        } else {
            navigator.clipboard.writeText(url).then(() => auth.showToast('Restaurant link copied!'));
        }
    },
                ...
                if (isPage) {
                            ...
                            <div class="d-flex gap-3 w-100">
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="mt-3">
                                        ${qty > 0 ? `
                                            <div class="d-flex align-items-center gap-2">
                                                <button class="btn btn-sm btn-outline-primary rounded-circle" onclick="Foodz.updateCart('${child.key}', -1)">-</button>
                                                <span class="fw-bold">${qty}</span>
                                                <button class="btn btn-sm btn-outline-primary rounded-circle" onclick="Foodz.updateCart('${child.key}', 1)">+</button>
                                            </div>
                                        ` : `
                                            <button class="add-btn" onclick="Foodz.updateCart('${child.key}', 1)" ${stock <= 0 ? 'disabled' : ''}>
                    ...
                    div.innerHTML = `
                        ...
                        <div>
                            ...
                            ` : `
                                <button class="add-btn" onclick="Foodz.updateCart('${child.key}', 1)" ${stock <= 0 ? 'disabled' : ''}>${stock <= 0 ? 'Sold Out' : '+ Add'}</button>
                            `}
                        </div>
                    `;
                }
                        ...
                        <div class="d-flex gap-3 w-100">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <div class="mt-3">
                                    ${qty > 0 ? `
                                        <div class="d-flex align-items-center gap-2">
                                            <button class="btn btn-sm btn-outline-primary rounded-circle" onclick="Foodz.updateCart('${item.id}', -1)">-</button>
                                            <span class="fw-bold">${qty}</span>
                                            <button class="btn btn-sm btn-outline-primary rounded-circle" onclick="Foodz.updateCart('${item.id}', 1)">+</button>
                                        </div>
                                    ` : `
                                        <button class="add-btn" onclick="Foodz.updateCart('${item.id}', 1)" ${stock <= 0 ? 'disabled' : ''}>
                                            ${stock <= 0 ? 'Sold Out' : '+ Add'}
                                        </button>
                                    `}
                                </div>
    ...
    validateCartRestaurantsAvailable: async function() {
        ...
        if (offlineNames.length) {
            auth.showToast('Sorry! ' + offlineNames.join(', ') + ' is currently Offline. Please order from an online restaurant.');
            return false;
        }
    ...
    ensureItemRestaurantAvailable: async function(itemId) {
        ...
        const restaurant = snap.val() || {};
        if (restaurant.isOnline === false) {
            auth.showToast('Sorry! ' + (restaurant.name || 'This restaurant') + ' is currently Offline. Please order from an online restaurant.');
            return false;
        }
    ...
    _checkSameRestaurant: function(newSellerId) {
        if (!newSellerId || this.cart.length === 0) return true;
        const existingSellerIds = new Set(this.cart.map(item => item.sellerId || item.restaurantId).filter(Boolean));
        if (existingSellerIds.size > 0 && !existingSellerIds.has(newSellerId)) {
            auth.showToast('⚠️ You can only order from ONE restaurant at a time! Please clear cart first or order separately.');
            return false;
        }
        return true;
    },
    ...
    updateCart: async function(itemId, change) {
        ...
        // Before adding new item, check restaurant consistency
        if (change > 0) {
            const itemSnap = await db.ref('products/' + itemId).once('value');
            if (!itemSnap.exists()) return;
            const itemData = itemSnap.val();
            const newSellerId = itemData.sellerId;
            if (!this._checkSameRestaurant(newSellerId)) return;
        }
        ...
        if (idx > -1) {
            ...
            if (!snap.exists()) return;
            const item = snap.val();
            if (Number(item.stock || 0) <= 0) { auth.showToast('Item is sold out'); return; }
            const img = (item.images && item.images.length > 0) ? item.images[0] : item.image;
            this.cart.push({
                id: itemId,
                name: item.name,
    ...
    applyCoupon: async function() {
        ...
        try {
            // First check the 'coupons' node (seller-created coupons)
            let found = null;
            const couponsSnap = await db.ref('coupons').orderByChild('code').equalTo(code).once('value');
            if (couponsSnap.exists()) {
                couponsSnap.forEach(c => {
                    const coupon = c.val();
            ...
            // If not found in 'coupons', check the 'offers' node (admin-created platform/restaurant offers)
            if (!found) {
                const offersSnap = await db.ref('offers').once('value');
                if (offersSnap.exists()) {
                    offersSnap.forEach(c => {
                        const offer = c.val() || {};
                        // Match by coupon code (case-insensitive)
                        const offerCode = (offer.coupon || '').toString().trim().toUpperCase();
                        if (offerCode !== code) return;
                        const expired = offer.expiry ? new Date(offer.expiry).getTime() < Date.now() : false;
                        if (offer.active === false || expired) return;
                        // Convert offer to coupon-like object
                        found = {
                            name: offer.title || offer.coupon || 'Offer',
                            ...
                            active: true,
                            restaurantId: offer.type === 'restaurant' ? (offer.restaurantId || null) : null
                        };
                    });
                }
            }
    ...
    updateCouponSummary: function(total) {
        // Update Order Summary section (top card)
        const discountRow = document.getElementById('coupon-discount-row');
        const discount = document.getElementById('coupon-discount');
        if (discountRow) discountRow.style.display = this.couponDiscount > 0 ? 'flex' : 'none';
        if (discount) discount.textContent = `- ₹${this.couponDiscount.toFixed(2)}`;

        // Update Checkout Modal footer
        const modalCouponRow = document.getElementById('checkout-coupon-row');
        const modalCouponAmt = document.getElementById('checkout-coupon-amount');
        if (modalCouponRow) modalCouponRow.style.display = this.couponDiscount > 0 ? 'flex' : 'none';
        if (modalCouponAmt) modalCouponAmt.textContent = `- ₹${this.couponDiscount.toFixed(2)}`;
    },
    ...
    updateCheckoutTotals: function() {
        ...
        // Update Order Summary section (top card on cart.html)
        const subtotalEl = document.getElementById('cart-subtotal');
        if (subtotalEl) subtotalEl.textContent = '₹' + itemTotal.toFixed(2);

        const deliveryPageEl = document.getElementById('delivery-fee-display');
        if (deliveryPageEl) deliveryPageEl.textContent = '₹' + this.deliveryFee.toFixed(2);

        const payableEl = document.getElementById('payable-total');
        if (payableEl) payableEl.textContent = '₹' + payable.toFixed(2);

        // Update Checkout Modal footer (unique IDs)
        const modalCartTotalEl = document.getElementById('checkout-cart-total-sum');
        if (modalCartTotalEl) modalCartTotalEl.textContent = 'Total: ₹' + itemTotal.toFixed(2);

        const modalDeliveryFeeEl = document.getElementById('checkout-delivery-fee-sum');
        if (modalDeliveryFeeEl) modalDeliveryFeeEl.textContent = '₹' + this.deliveryFee.toFixed(2);

        const modalPayableEl = document.getElementById('checkout-payable-sum');
        if (modalPayableEl) modalPayableEl.textContent = '₹' + payable.toFixed(2);
    },
        ...
        if (this.cart.length === 0) {
            ...
            container.innerHTML = this.cart.map((item, i) => `
                <div class="checkout-item">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <button class="btn btn-sm btn-outline-danger rounded-circle" onclick="Foodz.updateCartFromModal(${i}, -1)">-</button>
                        <span class="fw-bold">${item.qty}</span>
                        <button class="btn btn-sm btn-outline-primary rounded-circle" onclick="Foodz.updateCartFromModal(${i}, 1)">+</button>
                        <button class="btn btn-sm btn-outline-danger ms-2" onclick="Foodz.removeCartItem(${i})"><i class="fas fa-trash"></i></button>
                    </div>
                </div>
            `).join('');
            ...
            <div class="card shadow-sm mb-3" style="border-radius:16px;">
                <div class="card-body">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <button class="btn btn-sm btn-outline-danger rounded-circle" onclick="Foodz.updateCartFromCartPage(${i}, -1)">-</button>
                            <span class="fw-bold" style="min-width:24px;text-align:center;">${item.qty}</span>
                            <button class="btn btn-sm btn-outline-primary rounded-circle" onclick="Foodz.updateCartFromCartPage(${i}, 1)">+</button>
                            <button class="btn btn-sm btn-outline-danger ms-2" onclick="Foodz.removeCartItemFromCartPage(${i})"><i class="fas fa-trash"></i></button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    proceedToCheckoutFromCart: async function() {
        ...
        if (this.cart.length === 0) { auth.showToast('Your cart is empty'); return; }
    ...
    placeOrder: async function() {
        if (!this.currentUser) { auth.showToast('Please login first'); return; }
        if (this._orderPlacing) { auth.showToast('⏳ Order is already being placed...'); return; }
        this._orderPlacing = true;
        ...
        // ===== Delivery service check (Ismailabad only) =====
        // Avoid fake orders: require location + ensure within defined radius.
        const geoStatus = document.getElementById('geo-status');
        const hintEl = document.getElementById('service-area-hint');
        ...
        // Clear previous geo message
        if (geoStatus) geoStatus.innerHTML = '';



        if ((!lat || !lng) && localStorage.getItem('foodz-last-location')) {
            try {
                const saved = JSON.parse(localStorage.getItem('foodz-last-location'));
                lat = lat || saved.lat;
                lng = lng || saved.lng;
                if (lat && lng) {
                    document.getElementById('checkout-lat').value = lat;
        ...
        if (!name || !phone || !resolvedAddress || !city || !pincode) {
            auth.showToast('Please fill all required fields');
            return;
        }

        if (!lat || !lng) {
            auth.showToast('Please enable location access and get current location to place an order.');
            return;
        }

        // ===== Delivery zone polygon check =====
        // Check if the delivery location falls within the defined service area polygon
        const deliveryPoint = { lat: Number(lat), lng: Number(lng) };
        if (isNaN(deliveryPoint.lat) || isNaN(deliveryPoint.lng)) {
            auth.showToast('Invalid delivery location. Please try again.');
            return;
        }
        if (!isPointInPolygon(DELIVERY_POLYGON, deliveryPoint)) {
            if (geoStatus) {
                geoStatus.innerHTML = `<span class="text-danger"><i class="fas fa-ban me-1"></i>Invalid delivery location</span>`;
            }
            if (hintEl) {
                hintEl.className = 'alert alert-danger py-2 mb-2';
                hintEl.innerHTML = 'Sorry! Delivery is available only within the defined service area. Your location is outside the delivery zone.';
            }
            auth.showToast('Sorry! Invalid delivery location. Delivery is available only within the defined service area.');
            return;
        }
        ...
        try {
            ...
            // ===== Split orders per restaurant (Zomato/Swiggy style) =====
            // Group cart items by seller/restaurant
            const groups = {};
            for (const it of this.cart) {
                const rid = it.sellerId || it.restaurantId || '';
                if (!rid) continue;
            ...
            if (!restaurantIds.length) {
                auth.showToast('Cart items could not be matched to any restaurant.');
                return;
            }

            // Coupon: apply only if coupon.restaurantId is null/empty OR matches group restaurant.
            const couponRestaurantId = this.coupon?.restaurantId || null;

            // Place one order per restaurant
            const orderIds = [];
            for (const rid of restaurantIds) {
                // Fetch restaurant data per group
                const restSnap = await db.ref('restaurants/' + rid).once('value');
                const rest = restSnap.val() || {};
                if (rest.isOnline === false) {
                    auth.showToast('Sorry! ' + (rest.name || 'This restaurant') + ' is currently Offline.');
                    return;
                }

                const groupItems = groups[rid];
                const groupSubtotal = groupItems.reduce((s, i) => s + (i.price * i.qty), 0);

                // delivery fee per restaurant
                const groupDeliveryFee = Number(rest.deliveryFee || 0);

                // coupon discount per group (only if coupon matches)
                let groupCouponDiscount = 0;
                let couponCode = this.coupon ? (this.coupon.code || '') : '';
                if (this.coupon && (couponRestaurantId === null || couponRestaurantId === '' || couponRestaurantId === rid)) {
                    const minOrder = Number(this.coupon.minOrder || 0);
                    if (groupSubtotal >= minOrder) {
                        const discount = this.coupon.discountType === 'percent'
                            ? Math.min(Number(this.coupon.discountValue || 0), 200) / 100 * groupSubtotal
                            : Number(this.coupon.discountValue || 0);
                        groupCouponDiscount = Math.min(discount, groupSubtotal);
                    }
                }

                const groupFinal = Math.max(0, groupSubtotal + groupDeliveryFee - (groupCouponDiscount || 0));
                ...
                await orderRef.set({
                    ...
                    couponDiscount: groupCouponDiscount || 0,
                ...
                // Optional: notify customer per restaurant order
                try {
                    await db.ref('notifications').push({
                        orderId: orderRef.key,
                        userId: this.currentUser.uid,
                        message: '✅ Your order has been placed for ' + (rest.name || 'Restaurant') + '.',
                        timestamp: firebase.database.ServerValue.TIMESTAMP,
                        read: false
                    });
                } catch (e) {}

                // EmailJS: order placed email already send hota hai (existing behavior).
                // Delivered hone ke baad email bhejne ka implementation customer-orders.html me done hoga.
            ...
            const cartItemsListLegacy = document.getElementById('cart-items');
            if (cartList) cartList.innerHTML = '';
            if (cartItemsListLegacy) cartItemsListLegacy.innerHTML = '';
            const emptyState = document.getElementById('cart-empty');
            if (emptyState) emptyState.classList.remove('d-none');
            auth.showToast('🎉 Order placed successfully! Track it in My Orders.');
...
// Fallback radius for distance-based filtering (product index display)
const MAX_DELIVERY_RADIUS_KM = 4;

// Delivery zone polygon (Ismailabad service area)
// Coordinates are in GeoJSON format [lng, lat]
const DELIVERY_POLYGON = [
    [76.65703076846498, 30.1168749482341],
    [76.65705250916993, 30.117473356352328],
    [76.65711764548409, 30.118069399223092],
    [76.6572259203442, 30.118660724601376],
    [76.65737690643888, 30.119244998901525],
    [76.65757000789498, 30.11981991640511],
    [76.65780446262924, 30.120383208358273],
    [76.65807934535599, 30.120932651922598],
    [76.65839357123865, 30.12146607894413],
    [76.65874590017124, 30.12198138450632],
    [76.65913494167239, 30.122476535232625],
    [76.65955916037309, 30.122949577306745],
    [76.6600168820759, 30.12339864417836],
    [76.66050630036236, 30.123821963924375],
    [76.66102548372211, 30.12421786623654],
    [76.66157238317562, 30.124584789007965],
    [76.6621448403606, 30.124921284492807],
    [76.66274059605004, 30.125226025014502],
    [76.66335729906847, 30.125497808200624],
    [76.66399251557077, 30.1257355617233],
    [76.66464373864763, 30.125938347526873],
    [76.66530839821907, 30.12610536552605],
    [76.66598387117745, 30.126235956760013],
    [76.66666749173955, 30.12632960599019],
    [76.66735656196734, 30.12638594373135],
    [76.66804836241545, 30.126404747708165],
    [76.66874016286356, 30.12638594373135],
    [76.66942923309136, 30.12632960599019],
    [76.67011285365346, 30.126235956760013],
    [76.67078832661183, 30.12610536552605],
    [76.67145298618327, 30.125938347526873],
    [76.67210420926013, 30.1257355617233],
    [76.67273942576243, 30.125497808200624],
    [76.67335612878085, 30.125226025014502],
    [76.67395188447031, 30.124921284492807],
    [76.67452434165527, 30.124584789007965],
    [76.67507124110878, 30.12421786623654],
    [76.67559042446854, 30.123821963924375],
    [76.676079842755, 30.12339864417836],
    [76.67653756445782, 30.122949577306745],
    [76.6769617831585, 30.122476535232625],
    [76.67735082465967, 30.12198138450632],
    [76.67770315359226, 30.12146607894413],
    [76.67801737947492, 30.120932651922598],
    [76.67829226220167, 30.120383208358273],
    [76.67852671693593, 30.11981991640511],
    [76.67871981839203, 30.119244998901525],
    [76.67887080448669, 30.118660724601376],
    [76.67897907934682, 30.118069399223092],
    [76.67904421566098, 30.117473356352328],
    [76.67906595636592, 30.1168749482341]
];

// Point-in-polygon using Ray Casting algorithm
// polygon: array of [lng, lat] pairs (GeoJSON format)
// point: { lat, lng }
...
function getLocation() {
    Foodz.getCurrentLocation(true);
}

// ====== INIT ======
document.addEventListener('DOMContentLoaded', () => Foodz.init());
