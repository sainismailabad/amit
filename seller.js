...
const Seller = {
  ...
  loadSettings: async function () {
    ...
    set('rest-mobile', r.mobile || '');
    set('rest-whatsapp', r.whatsapp || '');
  ...
  saveSettings: async function () {
    const data = {
      ...
      mobile: document.getElementById('rest-mobile').value || '',
      whatsapp: document.getElementById('rest-whatsapp').value || '',
...
// ===== PRODUCTS =====
    ...
    try {
      ...
      for (const order of active) {
        ...
        tr.innerHTML = `
          ...
          <td>
            ${itemsHtml}
            ${order.orderNotes ? `<div class="mt-1 p-1" style="background:#fff3cd;border-radius:4px;font-size:0.7rem;color:#856404;border:1px solid #ffeeba;"><i class="fas fa-pen me-1"></i>${order.orderNotes}</div>` : ''}
          </td>
  ...
  updateStatus: async function (orderId, currentStatus) {
    ...
    if (status === 'Cancelled' || status === 'Delivered') {
      auth.showToast('Order already finalized: ' + status);
      return;
    }
    ...
    const currentColor = statusColors[status] || '#888';
    const nextColor = statusColors[next] || '#888';
    ...
    const finalAmount = Number(order.finalAmount ?? Math.max(0, itemTotal + deliveryFee + additionalCharges - couponDiscount));
    
    const itemsHtml = (order.items || []).map(item =>
      `<div style="display:flex;justify-content:space-between;padding:4px 0;font-size:0.85rem;">
        <span>${item.name || 'Item'} x${item.qty || 1}</span>
        <span style="font-weight:600;">₹${Number(item.price || 0).toFixed(2)}</span>
      </div>`
    ).join('');
    ...
    // WhatsApp message for customer - order details
    const customerMsg = encodeURIComponent(
      `🍕 *Order Update - Ibad Foodz* 🍕\n\n` +
      `Hi ${customerName}! 👋\n\n` +
      `📋 *Order #${orderIdShort}*\n` +
      `Status: *${status} → ${next}*\n\n` +
      `🛵 *Items Ordered:*\n${(order.items || []).map(i => `• ${i.name} x${i.qty} - ₹${Number(i.price*i.qty).toFixed(2)}`).join('\n')}\n\n` +
      `💰 *Total: ₹${finalAmount.toFixed(2)}*\n` +
      `📍 *Delivery:* ${order.deliveryAddress || ''}\n\n` +
      `🙏 Thank you for ordering with Ibad Foodz!`
    );
    
    // WhatsApp message for delivery boy - customer details
    const dbName = order.deliveryPartnerName || 'Delivery Partner';
    const dbMsg = encodeURIComponent(
      `🛵 *Delivery Assignment - Ibad Foodz* 🛵\n\n` +
      `Hi ${dbName}! 👋\n\n` +
      `📋 *Order #${orderIdShort}*\n` +
      `Status: *${status} → ${next}*\n\n` +
      ...
      `💰 *Amount to Collect: ₹${finalAmount.toFixed(2)}*\n\n` +
      `🏪 *Restaurant:* ${order.restaurantName || 'Restaurant'}\n` +
      `📦 *Items:* ${(order.items || []).map(i => `${i.name} x${i.qty}`).join(', ')}\n\n` +
      `⚡ Please deliver as soon as possible!`
    );
    
    // Load restaurant contact info for the Call button
    let restMobile = '';
    let restWhatsapp = '';
    try {
      const restSnap = await db.ref('restaurants/' + this.currentUser.uid + '/mobile').once('value');
      restMobile = restSnap.val() || '';
      const waSnap = await db.ref('restaurants/' + this.currentUser.uid + '/whatsapp').once('value');
      restWhatsapp = waSnap.val() || '';
    } catch (e) {}

    const specialNotes = order.orderNotes || '';
    const notesHtml = specialNotes ? `
      <div style="background:#fff3cd;border-radius:12px;padding:14px;margin-bottom:14px;border:1px solid #ffeeba;">
        <div style="font-size:0.75rem;color:#856404;margin-bottom:4px;"><i class="fas fa-pen me-1"></i>Special Instructions</div>
        <div style="font-size:0.85rem;color:#856404;font-weight:600;">${specialNotes}</div>
      </div>
    ` : '';

    const restPhoneHtml = (restMobile || restWhatsapp) ? `
      <div style="background:#e8f5e9;border-radius:12px;padding:14px;margin-bottom:14px;border:1px solid #c8e6c9;">
        <div style="font-size:0.75rem;color:#2e7d32;margin-bottom:8px;"><i class="fas fa-store me-1"></i>Restaurant Contact</div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;">
          ${restMobile ? `
            <a href="tel:${restMobile}" style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px;background:#4caf50;color:white;padding:12px;border-radius:12px;text-decoration:none;font-weight:700;font-size:0.85rem;">
              <i class="fas fa-phone-alt"></i> Call Restaurant
            </a>
          ` : ''}
          ${restWhatsapp ? `
            <a href="https://wa.me/91${restWhatsapp}" target="_blank" style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px;background:#25D366;color:white;padding:12px;border-radius:12px;text-decoration:none;font-weight:700;font-size:0.85rem;">
    ...
    const refEl = document.getElementById('status-modal-order-ref');
    refEl.textContent = `Order #${orderIdShort}`;
    
    body.innerHTML = `
      <!-- Current & Next Status -->
      <div style="display:flex;align-items:center;justify-content:center;gap:16px;margin-bottom:20px;">
        <div style="background:${currentColor};color:white;padding:10px 20px;border-radius:999px;font-weight:700;font-size:0.95rem;">
          ${status}
        </div>
        <div style="font-size:1.5rem;color:#888;">→</div>
        <div style="background:${nextColor};color:white;padding:10px 20px;border-radius:999px;font-weight:700;font-size:0.95rem;">
          ${next}
        </div>
      </div>
      ...
      <div style="background:#f8f9fa;border-radius:12px;padding:14px;margin-bottom:14px;">
        <div style="font-size:0.75rem;color:#888;margin-bottom:4px;">👤 Customer</div>
        <div style="font-weight:700;">${customerName}</div>
        ${customerPhone ? `<div style="font-size:0.85rem;color:#555;">📞 ${customerPhone}</div>` : ''}
        ${order.deliveryAddress ? `<div style="font-size:0.85rem;color:#555;margin-top:4px;">📍 ${order.deliveryAddress}${order.city ? ', '+order.city : ''}</div>` : ''}
      </div>

      <!-- Items Summary -->
      <div style="background:#f8f9fa;border-radius:12px;padding:14px;margin-bottom:14px;">
        <div style="font-size:0.75rem;color:#888;margin-bottom:4px;">📋 Items</div>
        ${itemsHtml || '<div style="color:#888;text-align:center;">No items</div>'}
        <div style="border-top:1px solid #e0e0e0;margin-top:6px;padding-top:6px;display:flex;justify-content:space-between;font-weight:700;">
          <span>Total</span>
          <span style="color:#16a34a;">₹${finalAmount.toFixed(2)}</span>
        </div>
      </div>
      ...
      <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:14px;">
        <button class="btn btn-success btn-lg w-100" onclick="Seller.doUpdateStatus('${orderId}','${next}')" style="border-radius:14px;font-weight:700;padding:14px;">
          <i class="fas fa-check-circle me-2"></i>Update to ${next}
        </button>
      </div>
      ...
      <!-- WhatsApp Buttons -->
      <div style="font-size:0.85rem;color:#888;margin-bottom:10px;text-align:center;">
        <i class="fab fa-whatsapp me-1" style="color:#25D366;"></i> Send Updates via WhatsApp
      </div>
      <div style="display:flex;gap:10px;flex-wrap:wrap;">
        ${customerPhone ? `
          <a href="https://wa.me/91${customerPhone}?text=${customerMsg}" target="_blank" class="btn btn-success" style="flex:1;border-radius:12px;font-weight:700;padding:12px;display:flex;align-items:center;justify-content:center;gap:8px;background:#25D366;border-color:#25D366;">
            <i class="fab fa-whatsapp" style="font-size:1.2rem;"></i> 📱 Customer
          </a>
        ` : ''}
        ${order.deliveryPartnerId ? `
          <a href="https://wa.me/91${order.deliveryPartnerPhone || ''}?text=${dbMsg}" target="_blank" class="btn btn-success" style="flex:1;border-radius:12px;font-weight:700;padding:12px;display:flex;align-items:center;justify-content:center;gap:8px;background:#25D366;border-color:#25D366;">
            <i class="fab fa-whatsapp" style="font-size:1.2rem;"></i> 🛵 Delivery Boy
          </a>
        ` : `<div style="flex:1;text-align:center;padding:12px;background:#f1f1f1;border-radius:12px;color:#888;font-size:0.85rem;">
          <i class="fas fa-motorcycle me-1"></i> No delivery partner assigned yet
        </div>`}
      </div>
    `;
  ...
  doUpdateStatus: async function(orderId, nextStatus) {
    try {
      await db.ref('orders/' + orderId).update({ status: nextStatus });
      ...
      auth.showToast('✅ Order status updated to ' + nextStatus);
    } catch (e) { auth.showToast('Error: ' + (e?.message || e)); }
  },
  ...
  loadHistory: async function () {
    ...
    try {
      ...
      for (const order of orders) {
        ...
        const safeStatus = String(status).toLowerCase().replace(/\s+/g, '-');
        const tr = document.createElement('tr');
        tr.innerHTML = `<td><strong>${order.customerName || 'Unknown'}</strong><br><small class="text-muted">${order.customerPhone || 'N/A'}</small></td><td>${itemsHtml}</td><td><small>${order.deliveryAddress || order.address || 'N/A'}</small></td><td><span class="status-badge status-${safeStatus}">${status}</span></td><td><small>${d}</small></td><td>${status === 'Delivered' ? `<button class="btn btn-sm btn-outline-primary" onclick="Invoice.download('${order.id}', 'seller')"><i class="fas fa-file-invoice"></i> Download Invoice</button>` : ''}</td>`;
        tbody.appendChild(tr);
      }
      if (tbody.children.length === 0) tbody.innerHTML = '<tr><td colspan="6" class="text-center py-4 text-muted">No order history</td></tr>';
    } catch (e) { tbody.innerHTML = `<tr><td colspan="6" class="text-danger">Error: ${e?.message || e}</td></tr>`; }
  }
};
